...
import { FaShoppingCart, FaTag, FaClock } from "react-icons/fa";
...
export default function DashboardOffers() {
  ...
  const handleAddToCart = (offer) => {
    if (offer.producto) {
      add(offer.producto, 1);
      enqueueSnackbar(`${offer.producto.nombre} añadido al carrito`, {
        variant: "success",
          ...
          {offers.map((offer, index) => (
            ...
            >
              ...
              <div className="p-6">
                ...
                {offer.producto && (
                  <div className="bg-amber-50 rounded-lg p-4 mb-4 border border-amber-100">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-[#8D6E63] mb-1">Producto</p>
                        <p className="font-semibold text-[#5D4037]">
                          {offer.producto.nombre}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-[#8D6E63] mb-1">Precio</p>
                ...
                {/* Dates */}
                <div className="flex items-center gap-4 text-sm text-[#8D6E63] mb-4">
                  <div className="flex items-center gap-2">
                    <FaClock />
                    <span>Válida desde: {formatDate(offer.fecha_inicio)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <FaClock />
                    <span>Hasta: {formatDate(offer.fecha_fin)}</span>
                  </div>
                </div>
